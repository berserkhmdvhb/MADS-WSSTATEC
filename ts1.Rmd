...
```{r}
autoplot(forecast(forecast_ts_train_meantemp)) + autolayer(ts_test_meantemp)
```
...
```{r}
VAR_est_weekly <- VAR(y = v1, season=4, type="both")
VAR_est_weekly
...
```{r}
summary(VAR_est_weekly)

```

```{r}
summary(VAR_est$varresult$ts_train_meantemp)$adj.r.squared
```

```{r}
summary(VAR_est$varresult$ts_train_windspeed)$adj.r.squared
```

```{r}
summary(VAR_est_weekly$varresult$meantemp)$adj.r.squared
```

```{r}
summary(VAR_est_weekly$varresult$wind_speed)$adj.r.squared
```

```{r}
forecasts <- predict(VAR_est_weekly)
plot(forecasts)
```
...
```{r}
Granger_meantemp <- causality(VAR_est_weekly, cause = "meantemp")
Granger_meantemp
```

```{r}
Granger_windspeed <- causality(VAR_est_weekly, cause = "wind_speed")
Granger_windspeed
```


```{r}
FEVD1 <- fevd(VAR_est_weekly, n.ahead = 10)
FEVD1
plot(FEVD1)
```
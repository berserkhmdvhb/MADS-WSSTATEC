...
For this, after importing the dataset, outliers are removed in [Preprocessing Section](#preprocess) section. Then meantemp column is assigned to a time series object in [Time Series Construct]{#initts} section for further processing, analysis, and forecast.
...
1. Forecast original time series of meantemp:
```{r}
...
```{r}
autoplot(forecast(forecast_ts_train))
```

...
For this, after importing the dataset, outliers are removed in [Preprocessing Section](#preprocess). Then meantemp column is assigned to a time series object in [Time Series Construct]{#initts} for further processing, analysis, and forecast.
...
## **Correlation Plots**
```{r}
ggAcf(ts_mon_train, lag=50)
pacf (ts_mon_train, lag=50, pl = TRUE)
```
...
Forecast original time series of meantemp:
```{r}


forecast_ts_train = auto.arima(ts_train,
                            trace = TRUE, 
                            seasonal= FALSE,
                            stepwise=FALSE,
                            approximation=FALSE)
checkresiduals(forecast_ts_train)
```
...
Forecast deseasonalized and detrended time series:
```{r}
...
`r colorize("Q4. I did all unit root tests and ARIMA models on all the following datasets: original time series, deseasonalized time series, and combination of deaseonalized and detrended time series. Could you please advise me with the parts I need to change or improve?.", "red")`

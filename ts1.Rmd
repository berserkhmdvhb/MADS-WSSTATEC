...
fit_windspeed = nnetar(xts_train_windspeed)
...
```{r}
nnetforecast <- forecast(fit_windspeed, h = 114, PI = F)  #Prediction intervals do not come by default in neural net forecasts, in constrast to ARIMA or exponential smoothing model
autoplot(nnetforecast) + theme(plot.title = element_text(hjust = 0.5))
```

```{r}

fit_meantemp = nnetar(xts_train_meantemp)
...
```{r}
nnetforecast <- forecast(fit_meantemp, h = 114, PI = F)  #Prediction intervals do not come by default in neural net forecasts, in constrast to ARIMA or exponential smoothing model
autoplot(nnetforecast) + theme(plot.title = element_text(hjust = 0.5))
```

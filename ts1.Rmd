...
```{r}
nnetforecast_windpseed <- forecast(fit_windspeed, h = 114, PI = T)  #Prediction intervals do not come by default in neural net forecasts, in constrast to ARIMA or exponential smoothing model
nnetforecast_windspeed
```



```{r}
forecast_windspeed <- nnetforecast_windspeed |> forecast(h=114, PI = T)
forecast_windspeed 
```
...
nnetforecast_meantemp <- forecast(fit_meantemp, h = 114, PI = T)
nnetforecast_meantemp
```

```{r}
forecast_meantemp <- nnetforecast_meantemp |> forecast(h=114, PI = T)
forecast_meantemp
```
...
```{r}
forecast_meantemp |> autoplot() + autolayer(ts_test_windspeed)
```
